...
export default function ExamPage() {
  ...
  useEffect(() => {
    ...
    const fetchExamData = async () => {
      try {
        // Fetch exam details
        const examRes = await fetch(`http://vijeta-api.onrender.com/api/exams/${examId}`);
        const examData = await examRes.json();
        setExamDetails(examData);

        // Fetch questions
        const res = await fetch(`http://vijeta-api.onrender.com/api/exams/${examId}/questions`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("http://vijeta-api.onrender.com/api/exam-attempts", {
        method: "POST",
        headers: { 
